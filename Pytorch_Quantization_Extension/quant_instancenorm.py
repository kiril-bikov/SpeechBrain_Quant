...
__all__ = [
    "QuantInstanceNorm1d", "QuantInstanceNorm2d", "QuantInstanceNorm3d", "QuantBatchNorm1d","QuantLayerNorm"
]
...
class QuantBatchNorm1d(BatchNorm1d, _utils.QuantInputMixin):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True, **kwargs):
        super(QuantBatchNorm1d, self).__init__(num_features, eps, momentum, affine,
                                               track_running_stats)
        quant_desc_input = _utils.pop_quant_desc_in_kwargs(self.__class__, input_only=True, **kwargs)
        self.init_quantizer(quant_desc_input)
    ...
    def forward(self, input):
        quant_input = self._input_quantizer(input)
        return super(QuantBatchNorm1d, self).forward(quant_input)



class QuantLayerNorm(LayerNorm, _utils.QuantInputMixin):
    ...
    def forward(self, input):
        quant_input = self._input_quantizer(input)
        return super(QuantLayerNorm, self).forward(quant_input)
