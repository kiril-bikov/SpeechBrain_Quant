...
class TensorQuantizer(nn.Module):
    ...
    def __init__(self, quant_desc=QuantDescriptor(), disabled=False, if_quant=True, if_clip=False, if_calib=False, rounding_decision=None):
        ...
        self.rounding_decision = rounding_decision
        #self.stochasting_rounding = False
    ...
    def _quant_forward(self, inputs):
        ...
        if self._fake_quant:
            if not TensorQuantizer.use_fb_fake_quant:
                if self.rounding_decision is None:
                    outputs = fake_tensor_quant(inputs, amax, self._num_bits, self._unsigned,
                                                        self._narrow_range)
                else:
                    outputs = fake_tensor_quant(inputs, amax, self._num_bits, self._unsigned,
                                                        self._narrow_range, self.rounding_decision)
    ...
    def set_rounding_decision(self, decision):
        """
        Set the rounding decision for each weight.
        The decision tensor should match the shape of the weights.
        """
        #if not isinstance(decision, torch.Tensor):
            #raise ValueError("Rounding decision must be a PyTorch tensor")
        self.rounding_decision = decision