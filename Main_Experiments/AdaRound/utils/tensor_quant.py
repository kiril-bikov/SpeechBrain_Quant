...
class FakeTensorQuantFunction(Function):
    ...
    @staticmethod
    def forward(ctx, inputs, amax, num_bits=8, unsigned=False, narrow_range=True, rounding_decision = None):
        ...
        def legacy_quant_func():
            ...
            outputs, scale = _tensor_quant(inputs, amax, num_bits, unsigned, narrow_range, rounding_decision)
...
def stochastic_round(x):
    """Stochastic rounding to the nearest integer."""
    floor_x = torch.floor(x)
    ceil_x = torch.ceil(x)
    prob = x - floor_x  # Probability to round up
    return torch.where(torch.rand_like(x) < prob, ceil_x, floor_x)

def custom_round(x, decision):
    """Custom rounding based on a rounding decision."""
    floor_x = torch.floor(x)
    ceil_x = torch.ceil(x)
    round_x = x.round_()
    return torch.where(decision, ceil_x, floor_x)
def _tensor_quant(inputs, amax, num_bits=8, unsigned=False, narrow_range=True, rounding_decision = None):
    ...
    # Stochastic rounding applied here
    #outputs = torch.clamp(stochastic_round(inputs * scale), min_bound, max_bound)

    if rounding_decision is None:
        outputs = torch.clamp((inputs * scale).round_(), min_bound, max_bound)
        #outputs = torch.clamp(stochastic_round(inputs * scale), min_bound, max_bound)
    else:
        # Rounding based on rounding_decision
        outputs = torch.clamp(custom_round(inputs * scale, rounding_decision), min_bound, max_bound)
    """
