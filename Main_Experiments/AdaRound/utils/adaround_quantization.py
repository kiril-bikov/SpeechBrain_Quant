...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # print('--------------------')
        # print(f'Before normalize: feats.shape = {feats.shape}')
        # print('--------------------')
        feats = self.modules.normalize(feats, wav_lens)
        # print(f'After normalize feats.shape = {feats.shape}')
        # print('--------------------')
...
def quantize(asr, train_data):
    ...
    quantized_layers.append(asr.modules['enc']['CNN']['block_0']['conv_1'].conv) #8  0.33
    names.append('block_0_conv_1')
    ...
    names.append('block_0_linear')
    asr.modules['enc']['DNN']['block_1']['linear'].w = quantize_layer(asr.modules['enc']['DNN']['block_1']['linear'].w) #8
    quantized_layers.append(asr.modules['enc']['DNN']['block_1']['linear'].w)
    names.append('block_1_linear')
    ...
    names.append('dec_attn.conv_loc')
    ...
    names.append('dec_attn.mlp_attn')
    ...
    names.append('dec_attn.mlp_enc')
    asr.modules['dec'].attn.mlp_loc = quantize_layer(asr.modules['dec'].attn.mlp_loc)
    quantized_layers.append(asr.modules['dec'].attn.mlp_loc) #8 0.32
    names.append('dec_attn.mlp_loc')
    ...
    names.append('seq_lin')


    with torch.no_grad():
        collect_stats(quantized_layers, train_data, num_batches=8, model=asr)
        compute_amax(quantized_layers, method="percentile", percentile=99.99)

    with open('alphas_new.pkl', 'rb') as f:
        alphas = pickle.load(f)

    for layer in alphas.keys():
        for i in range(len(names)):
            if names[i] == layer:
                quantized_layers[i].weight_quantizer.set_rounding_decision(alphas[layer] > 0)
                break
